...
본 연구는 개별 아날로로그 블록 수준의 schematic-level 검증을 중심으로 수행되었다. 향후에는 SAR logic 및 CDAC를 결합한 전체 SAR ADC 회로를 구성하고, post-layout parasitic, PVT variation, device mismatch를 포함한 조건에서 변환 정확도와 동작 안정성을 검증할 예정이다.
